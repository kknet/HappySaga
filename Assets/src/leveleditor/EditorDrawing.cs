...
namespace com.zonglv.minigame.editor {
    public enum ColliderTypeChoices {
        Polygon_Collider, Edge_Collider
    }
    public enum overlapHandlingChoices {
        Follow_The_Edge, Cut_After_Collision, None
    }

    public class EditorDrawing : MonoBehaviour {
        public bool mCanDrawLine;
        ...
        void Start() {
            ...
            pathLineRenderer.sortingLayerName = "UI";
        ...
        void AddMousePointer() {
            GameObject mouse_obj = new GameObject ("MousePointer");
            mouse_obj.transform.parent = transform;
            mouse_obj.AddComponent<CircleCollider2D> ();
            mouse_obj.AddComponent<TargetJoint2D> ();
            //mouse_obj.AddComponent<Rigidbody2D>();
        }
        ...
        void Update() {
            ...
            if (Input.GetMouseButtonDown (0)) {
                EditorFreezeMoving.freeze = true;
            ...
            if (Input.GetMouseButtonUp (0)) {
                EditorFreezeMoving.freeze = false;
                if (canDraw == true) {
                ...
                if (fixedPosition == false && clone) {
                    clone.gameObject.AddComponent<EditorFreezeMoving> ();
                } else {
        ...
        public void LoadOldLine(List<Vector3> points) {
            StarPrefab ();
            pathLineRenderer.SetPositions (points.ToArray());
            List<Vector2> pos = new List<Vector2> ();
            for(int i = 0; i < points.Count; i++) {
                Vector2 v2 = new Vector2 (points[i].x,points[i].y);
                pos.Add (v2);
            }
            pathEdgeCollider.points = pos.ToArray();
        ...
        /// </summary>
        public void StarPrefab() {
            clone = (GameObject)Instantiate (path, mousePointer.transform.position, Quaternion.identity, LevelEditorManager.instance.mLineGroup); // instantiate prefab
        ...
        void DrawVisibleLine() {
            ...
            if (Vector2.Distance (mousePointer.transform.position, newVerticies_[posCount]) > verticesDistance) {
                ...
                LevelEditorManager.instance.SetLength (pathLineRenderer.positionCount);
                newVerticies_.Add (mousePointer.transform.position - clone.transform.position);
                if (mouseHit == false) {
                    newVerticies.Add (mousePointer.transform.position - clone.transform.position);
